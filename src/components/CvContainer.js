...
class CvContainer extends Component {
  ...
  constructor() {
    ...
    this.state = {
      ...
      educationArray: [
        { id: uniqid(), schoolName: "", degreeObtained: "", timeFrame: "" },
      ],
      experienceArray: [
        {
          id: uniqid(),
  ...
  experienceChange(targetName, targetID, newValue) {
    this.setState({
      // map current state array to new edited array
      experienceArray: this.state.experienceArray.map((experienceElem) => {
  ...
  addSkill() {
    this.setState({
      skillArray: this.state.skillArray.concat({ id: uniqid(), userSkill: "" }),
    });
  }

  deleteSkill(targetID) {
    this.setState({
      skillArray: this.state.skillArray.filter((elem) => elem.id !== targetID),
    });
  }
